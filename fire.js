...
async function deleteFood(foodId) {
    try {
        ...
        if (!dbInstance.objectStoreNames.contains("foods")) {
            throw new Error("❌ Không tìm thấy bảng 'foods' trong IndexedDB!");
        }

        const transaction = dbInstance.transaction(["foods"], "readwrite");
        const store = transaction.objectStore("foods");
        store.delete(foodId); // ID dạng chuỗi nếu lấy từ Firestore
...
async function saveRevenue(foodName, totalAmount, quantity, tableId) {
    ...
    try {
        await setDoc(foodRef, { 
            ...
            tableId: tableId,
            timestamp: new Date().toISOString()  // hoặc sử dụng Timestamp.fromDate(new Date()) nếu bạn dùng Firestore Timestamp
        });

        console.log(`✅ Doanh thu món "${foodName}" đã được lưu với ID ${revenueId}!`);
    } catch (error) {
